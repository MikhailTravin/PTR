(() => {
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) {
                ...
                window.addEventListener("click", (e => {
                    ...
                    if (!target.closest(".icon-menu") && !target.closest(".menu__body")) {
                        document.documentElement.classList.remove("menu-open");
                        bodyUnlock();
                    }
                }));
        ...
        function selectCatalog() {
            ...
            if (optionMenus) optionMenus.forEach(((optionMenu, i) => {
                ...
                if (select) {
                    ...
                    window.addEventListener("click", (e => {
                        const target = e.target;
                        if (!target.closest(".select__options") && !target.closest(".select__menu") && !target.closest(".filter-catalog__remove") && !target.closest(".select__option")) optionMenu.classList.remove("_active");
                    }));
                ...
                if (selectOther) selectOptions.forEach((option => {
                    option.addEventListener("click", (function(e) {
                        if (sBtntext) {
                            if (null != e.target.classList.contains(".select__option-text")) {
        ...
        function headerScroll() {
            ...
            document.addEventListener("windowScroll", (function(e) {
                ...
                if (scrollTop >= scrollChange) !document.documentElement.classList.contains("_header-scroll") ? document.documentElement.classList.add("_header-scroll") : null; else document.documentElement.classList.contains("_header-scroll") ? document.documentElement.classList.remove("_header-scroll") : null;
        ...
        if (searchIcon) {
            ...
            window.addEventListener("click", (e => {
                const target = e.target;
                if (!target.closest(".search-header__inputs") && !target.closest(".search-header__button")) searchInput.classList.remove("_search-open");
            }));
        }
        ...
        if (galleryItems) {
            galleryItems.forEach((item => {
                item.addEventListener("mouseenter", (function(e) {
                    ...
                    let child_arr = [];
                    for (let i = 0; i < child_count; i++) child_arr.push(this.parentNode.children[i]);
                    let filter_child_arr = [];
                    for (let i = 0; i < child_count; i++) {
                        let curr_child = this.parentNode.children[i];
                        if (curr_child !== this) filter_child_arr.push(curr_child);
        ...
        if (productCardFavourites) productCardFavourites.forEach((item => {
            item.addEventListener("click", (function(e) {
                item.classList.toggle("_active");
            }));
        }));
        const selectInputs = document.querySelectorAll(".select__input > input");
        if (selectInputs) selectInputs.forEach((input => {
            input.addEventListener("change", (function(e) {
                let parent = e.target.parentNode;
                if (input.value >= 1) parent.classList.add("_active"); else parent.classList.remove("_active");
            }));
        }));
        window["FLS"] = false;
        isWebp();
